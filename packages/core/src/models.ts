...
/**
 * `crypto.randomUUID` and `crypto.subtle` only exist in a secure context, so they are
 * missing on the band's `http://<master-ip>:8788` page. `getRandomValues` is always there.
 */
