export function NotesIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M6.2 3.4h8.1L19 8.1v12.5H6.2zm1.8 1.8v13.6h9.2V9.2h-3.7V5.2z" />
      <path d="M8.8 12.1h6.4v1.6H8.8zm0 3.1h6.4v1.6H8.8z" />
    </svg>
  );
}

export function InfoIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M12 3.2a8.8 8.8 0 1 1 0 17.6 8.8 8.8 0 0 1 0-17.6zm0 1.8a7 7 0 1 0 0 14 7 7 0 0 0 0-14z" />
      <circle cx="12" cy="8" r="1.25" />
      <path d="M11.1 10.5h1.8V17h-1.8z" />
    </svg>
  );
}

export function PlayIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M8 5.2v13.6L19.2 12z" />
    </svg>
  );
}

export function PauseIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M6.5 5h4v14h-4zM13.5 5h4v14h-4z" />
    </svg>
  );
}

export function AddIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M10.6 4.5h2.8v6.1h6.1v2.8h-6.1v6.1h-2.8v-6.1H4.5v-2.8h6.1z" />
    </svg>
  );
}

export function StopIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <rect x="6" y="6" width="12" height="12" rx="1.5" />
    </svg>
  );
}

export function SelectIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M4 5.5h10.5v3H4zM4 10.5h16v3H4zM4 15.5h16v3H4z" />
    </svg>
  );
}

export function SaveIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M5 4.5h11.2L19.5 8v11.5H5zm2.2 1.8v5.4h9.2V6.3zm1.7 8.2v4.2h6.2v-4.2z" />
    </svg>
  );
}

export function ResetIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M12 4.2a7.8 7.8 0 1 0 7.45 9.95h-2.15A5.9 5.9 0 1 1 12 6.1v3.15L18.4 5.6 12 2.05z" />
    </svg>
  );
}

export function DeleteIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M9.2 4.5h5.6l.7 1.5H19v1.8H5V6h3.5zm1 5.2h1.8v7.2H10.2zm3.8 0h1.8v7.2h-1.8zM7.4 9.7h1.8v9.1h5.6V9.7h1.8v10.9H7.4z" />
    </svg>
  );
}

export function PlayNextIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M4 5.5v13l9.5-6.5L4 5.5z" />
      <rect x="15.5" y="5.5" width="3.2" height="13" rx="0.6" />
    </svg>
  );
}

export function SkipIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M3.4 5.6v12.8L13.2 12z" />
      <rect x="16.8" y="5.8" width="2.3" height="12.4" rx="0.5" transform="rotate(45 18 12)" />
      <rect x="16.8" y="5.8" width="2.3" height="12.4" rx="0.5" transform="rotate(-45 18 12)" />
    </svg>
  );
}

export function ViewIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path
        fillRule="evenodd"
        d="M8.6 4.4h6.8L20.2 21H3.8Zm1.5 2.8h3.8L16.8 19H7.2Z"
      />
      <path d="M11.15 3.2h1.7l4.7 14.6-1.7.55z" />
      <rect x="14.15" y="10.4" width="3.9" height="3.1" rx="0.5" transform="rotate(-20 16.1 11.95)" />
    </svg>
  );
}

export function NotationIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <ellipse cx="6.8" cy="18" rx="3.4" ry="2.25" transform="rotate(-32 6.8 18)" />
      <ellipse cx="15.4" cy="18" rx="3.4" ry="2.25" transform="rotate(-32 15.4 18)" />
      <rect x="9.35" y="4.6" width="1.7" height="13.4" />
      <rect x="17.95" y="4.6" width="1.7" height="13.4" />
      <path d="M9.35 4.35h11.55v2.45H9.35z" />
    </svg>
  );
}

export function EighthNoteIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <ellipse cx="8.2" cy="18.1" rx="3.5" ry="2.3" transform="rotate(-28 8.2 18.1)" />
      <rect x="11.15" y="4.2" width="1.75" height="13.8" />
      <path d="M12.9 4.2c3.4 1.1 5.8 3.4 6.2 6.6-.9-1.4-2.4-2.5-4.2-3.1v3.4c1.9.6 3.4 1.8 4.2 3.4-.5-3.6-3-6.3-6.2-7.4z" />
    </svg>
  );
}

export function PreparationIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path
        fillRule="evenodd"
        d="M9 3.2h6l.7 1.6H18c1 0 1.8.8 1.8 1.8v12.4c0 1-.8 1.8-1.8 1.8H6c-1 0-1.8-.8-1.8-1.8V6.6c0-1 .8-1.8 1.8-1.8h2.3zm1.4 1v2h3.2v-2zM7.6 11h8.8v1.6H7.6zm0 3.2h8.8v1.6H7.6zm0 3.2h6.2v1.6H7.6z"
      />
    </svg>
  );
}

export function MixerIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <rect x="4.2" y="4.5" width="2.6" height="15" rx="0.6" />
      <rect x="10.7" y="4.5" width="2.6" height="15" rx="0.6" />
      <rect x="17.2" y="4.5" width="2.6" height="15" rx="0.6" />
      <circle cx="5.5" cy="9.2" r="2.1" />
      <circle cx="12" cy="14.4" r="2.1" />
      <circle cx="18.5" cy="8.2" r="2.1" />
    </svg>
  );
}

export function SpeakerMonitorIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M5 2.8h14c1.1 0 2 .9 2 2v14.4c0 1.1-.9 2-2 2H5c-1.1 0-2-.9-2-2V4.8c0-1.1.9-2 2-2zm1.2 2v14.4h11.6V4.8z" />
      <circle cx="12" cy="14.2" r="3.7" />
      <circle cx="12" cy="7.6" r="1.4" />
    </svg>
  );
}

export function LockIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M8.2 10.2V8.1a3.8 3.8 0 0 1 7.6 0v2.1h1.7v10.2H6.5V10.2zm1.8 0h4V8.1a2 2 0 0 0-4 0z" />
    </svg>
  );
}

export function ChronometerIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <circle cx="12" cy="14" r="6.8" />
      <path d="M12 7.2V4.5M9.5 3.5h5M17.1 8.7l1.5-1.5M12 10.5v3.8l2.7 1.7" />
    </svg>
  );
}

export function AutoScrollIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M12 3.4 20.2 13h-4.4v7.6H8.2V13H3.8z" />
    </svg>
  );
}

export function EditSectionsIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path
        fillRule="evenodd"
        d="M3.7 4.3h16.6v15.4H3.7zm1.7 1.7v12h13.2v-12z"
      />
      <ellipse cx="10.05" cy="15.7" rx="3.5" ry="2.55" transform="rotate(-28 10.05 15.7)" />
      <rect x="12.7" y="6.55" width="1.85" height="9" />
      <path d="M14.55 6.55c3.35.15 5.15 2.55 4.55 6.05-1.05-.95-2.45-1.55-4.55-1.35z" />
    </svg>
  );
}
